...
export function createTournament(db,ids,title,draw=null){
  ...
  const t={id:uid(),queueFrom:0,revision:1,title:(title||'Турнир TT League').trim().slice(0,80),startedAt:new Date().toISOString(),endedAt:null,status:'active',ids:[...ids],names:Object.fromEntries(ids.map(id=>[id,nameOf(db,id)])),draw:[...order],rules:clone(RULES),matches:[],voided:[],current:null};
...
// Nonnegative degrees form a loop-free multigraph iff their sum is even
// and no degree exceeds the sum of all others. Repeated opponents are allowed.
const feasible = deficits => {const sum=deficits.reduce((a,b)=>a+b,0);return deficits.every(d=>Number.isInteger(d)&&d>=0)&&sum%2===0&&Math.max(...deficits)<=sum/2;};
export function balancePlan(t){
  if(t.status!=='active'||t.matches.length<Math.floor(t.ids.length/2))throw Error('Сначала завершите стартовые матчи.');
  const counts=Object.values(progress(t).games);let target=Math.max(...counts);
  while(!feasible(counts.map(n=>target-n)))target++;
  return {target,remaining:counts.reduce((sum,n)=>sum+target-n,0)/2};
}
export function balanceProgress(t){
  if(t.balanceTarget==null)return null;
  const counts=Object.values(progress(t).games);
  return {target:t.balanceTarget,remaining:counts.reduce((sum,n)=>sum+t.balanceTarget-n,0)/2};
}
export function startBalance(t){
  const plan=balancePlan(t);t.queueFrom??=t.matches.length;
  t.balanceTarget=plan.target;t.current=nextPair(t);t.revision++;return plan;
}
export function continueTournament(t){
  const plan=balanceProgress(t);
  if(t.status!=='active'||!plan||plan.remaining!==0)throw Error('Сначала закончите выравнивание.');
  t.balanceTarget=null;t.current=nextPair(t);t.revision++;
}
export function nextPair(t){
  if(t.status!=='active')return null;
  if(t.matches.length<Math.floor(t.ids.length/2)||t.matches.length<(t.queueFrom??Infinity))return legacyPair(t);
  const {lv,last,games}=progress(t),rank=id=>t.draw.indexOf(id);
  const order=(a,b)=>games[a]-games[b]||last[a]-last[b]||rank(a)-rank(b);
  const repeats=(a,b)=>t.matches.filter(m=>m.a===a&&m.b===b||m.a===b&&m.b===a).length;
  const distance=(a,b)=>Math.abs(LEVELS.indexOf(lv[a])-LEVELS.indexOf(lv[b]));
  const balancing=t.balanceTarget!=null;
  const eligible=t.ids.filter(id=>!balancing||games[id]<t.balanceTarget).sort(order);
  if(!eligible.length)return null;
  const pairs=[];
  for(let i=0;i<(balancing?eligible.length:1);i++)for(let j=i+1;j<eligible.length;j++){
    const a=eligible[i],b=eligible[j];
    if(balancing&&!feasible(t.ids.map(id=>t.balanceTarget-games[id]-(id===a||id===b?1:0))))continue;
    pairs.push({a,b});
  }
  pairs.sort((x,y)=>order(x.a,y.a)||games[x.b]-games[y.b]||distance(x.a,x.b)-distance(y.a,y.b)||repeats(x.a,x.b)-repeats(y.a,y.b)||last[x.b]-last[y.b]||rank(x.b)-rank(y.b));
  if(!pairs.length)throw Error('Не удалось составить очередь выравнивания.');
  const {a,b}=pairs[0];
  return {a,b,cross:lv[a]!==lv[b],opening:false,reason:balancing?'Выравнивание: до '+t.balanceTarget+' матчей каждому':'Приоритет: меньше сыгранных матчей'};
}
...
export function recordScore(t,aScore,bScore){
  ...
  const m={balanceTarget:t.balanceTarget??null,id:uid(),a:pair.a,b:pair.b,scoreA:aScore,scoreB:bScore,winner,levelA:lv[pair.a],levelB:lv[pair.b],points:t.rules.points[lv[winner]],at:new Date().toISOString(),reason:pair.reason,cross:pair.cross};
  t.matches.push(m);t.queueFrom??=t.matches.length;t.revision++;t.current=nextPair(t);return m;
}
export function undoLast(t){if(t.status!=='active'||!t.matches.length)throw Error('Нет результата для отмены.');const m=t.matches.pop();t.balanceTarget=m.balanceTarget??null;t.voided.push({...m,voidReason:'Отмена последнего результата',voidedAt:new Date().toISOString()});t.current={a:m.a,b:m.b,reason:m.reason,cross:m.cross,opening:m.levelA==='Common'};t.revision++;return m;}
...
export function league(db, tournaments=completed(db)){const map=Object.fromEntries(db.players.map(p=>[p.id,{id:p.id,name:p.name,points:0,titles:0,tournaments:0,games:0,wins:0,losses:0,scored:0,conceded:0}]));for(const t of tournaments){for(const r of standings(t)){const p=map[r.id];p.points+=r.seasonPoints;p.titles+=r.place===1?1:0;p.tournaments++;p.games+=r.games;p.wins+=r.wins;p.losses+=r.losses;p.scored+=r.scored;p.conceded+=r.conceded;}}return Object.values(map).map(p=>({...p,rate:p.games?p.wins/p.games:0})).sort((a,b)=>b.points-a.points||b.titles-a.titles||b.rate-a.rate||a.name.localeCompare(b.name,'ru'));}
...
export function validateDB(data){
  ...
  for(const t of data.tournaments){
    ...
    if(t.queueFrom!=null&&(!Number.isInteger(t.queueFrom)||t.queueFrom<0||t.queueFrom>10000))throw Error('Некорректная версия очереди.');
    const checkTarget=(target,replay)=>{if(target==null)return;if(!Number.isInteger(target)||target<0||target>20000||replay.matches.length<Math.floor(t.ids.length/2)||!feasible(Object.values(progress(replay).games).map(n=>target-n)))throw Error('Повреждён режим выравнивания.');};
    const replay={...clone(t),matches:[],status:'active',current:null,balanceTarget:null};
    for(const m of t.matches){replay.balanceTarget=m?.balanceTarget??null;checkTarget(replay.balanceTarget,replay);replay.current=nextPair(replay);const lv=levels(replay);if(!m||typeof m.id!=='string'||mids.has(m.id)||!t.ids.includes(m.a)||!t.ids.includes(m.b)||m.a===m.b||!validScore(m.scoreA,m.scoreB)||m.winner!==(m.scoreA>m.scoreB?m.a:m.b)||m.levelA!==lv[m.a]||m.levelB!==lv[m.b]||m.points!==RULES.points[lv[m.winner]]||!Number.isFinite(Date.parse(m.at))||m.a!==replay.current?.a||m.b!==replay.current?.b)throw Error('Матчи или начисления в копии повреждены.');mids.add(m.id);replay.matches.push(m);}
    ...
    checkTarget(t.balanceTarget,replay);replay.balanceTarget=t.balanceTarget??null;replay.current=nextPair(replay);
    if(t.status==='active'){active++;if((t.current?.a??null)!==(replay.current?.a??null)||(t.current?.b??null)!==(replay.current?.b??null))throw Error('Повреждён текущий матч.');}
